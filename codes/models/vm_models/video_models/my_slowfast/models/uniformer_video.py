...
model_path = '/data/Disk_A/rongchang/.cache/torch/hub/checkpoints'
model_path = {
...
class Uniformer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        attn_drop_rate = cfg.ATTENTION_DROPOUT_RATE
        drop_path_rate = cfg.DROP_DEPTH_RATE
        split = cfg.SPLIT
        std = cfg.STD
        ...
        checkpoint = self.get_pretrained_model(cfg)
        if checkpoint:
    ...
    def get_pretrained_model(self, cfg):
        if cfg.PRETRAIN_NAME:
            checkpoint = torch.load(model_path[cfg.PRETRAIN_NAME], map_location='cpu')
