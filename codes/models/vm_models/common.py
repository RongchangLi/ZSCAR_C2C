...
class Evaluator:
    ...
    def __init__(self, dset,model):
        ...
        self.closed_mask = torch.BoolTensor(masks)#for
    ...
    def generate_predictions(self, scores, obj_truth, bias=0.0, topk=5):  # (Batch, #pairs)
        ...
        mask = self.closed_mask.repeat(scores.shape[0], 1)#5124
    ...
    def evaluate_predictions(self, predictions, attr_truth, obj_truth, pair_truth, allpred, topk=1):
        ...
        #################### Calculating AUC
        scores = predictions['scores'] #unbiased  # size:n_sample, n_pairs
        # getting score for each ground truth class
        correct_scores = scores[torch.arange(scores.shape[0]), pair_truth][unseen_ind]

        # Getting top predicted score for these unseen classes# size:n_sample, 1
        ...
        # Getting matched classes at max bias for diff  size:n_sample, 1
        ...
        # Go to CPU
        allpred= {k: v.to('cpu') for k, v in allpred.items()}
